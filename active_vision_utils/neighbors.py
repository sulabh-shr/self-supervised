...
def get_hierarchy_neighbors(img_name, root_path=None, folder=None, ann=None,
                            hierarchy=2):
    neighbors_dict = {0: [img_name]}

    for h in range(1, hierarchy + 1):
        neighbors_dict[h] = []

        for prev_neighbor in neighbors_dict[h - 1]:
            already_counted = []

            for neighbors_list in neighbors_dict.values():
                already_counted += neighbors_list

            # Add only those which are not already present in previous hierarchy
            neighbors_dict[h] += [i for i in get_neighbors(prev_neighbor,
                                                           root_path=root_path,
                                                           folder=folder,
                                                           ann=ann)
        ...
        # Remove Duplicates
        neighbors_dict[h] = list(
            set(filter(lambda x: x != img_name, neighbors_dict[h])))
...
def distance_sort_nodes(root_path=None, folder=None, image_struct=None,
    ...
    return sorted_cluster_centers, sorted_clusters
